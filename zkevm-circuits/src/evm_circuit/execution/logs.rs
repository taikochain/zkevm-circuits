    ...
    fn assign_exec_step(
        ...
        for i in 0..4 {
            ...
            if i < topic_count {
                ...
                self.topic_selectors[i].assign(region, offset, Ok(F::one()))?;
                topic_stack_entry.1 += 1;
            } else {
                self.topic_selectors[i].assign(region, offset, Ok(F::zero()))?;
            }
            self.topics[i].assign(region, offset, Ok(topic))?;
        }
        ...
        self.is_static_call
            .assign(region, offset, Ok(F::from(call.is_static as u64)))?;
        self.is_persistent
            .assign(region, offset, Ok(F::from(is_persistent)))?;
        self.tx_id

...
fn bench_circuit() {
    ...
    let rows = CompressionCircuit::<Fr,R>::rows(&INPUTS);
    assert!((rows + R) > (1 << K), "The number of rounds must be the maximum for the chosen k, rows per round and compression input number!"); 
